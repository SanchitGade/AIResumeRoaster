...
const responseSchema = {
  ...
  properties: {
    ...
    issues: {
      type: Type.ARRAY,
      description: "Excatly 5 prioritized issues",
      items: {
        type: Type.OBJECT,
    ...
    strengths: {
      type: Type.ARRAY,
      description: "Excatly 5 strengths",
      items: {
        type: Type.OBJECT,
    ...
    bulletRewrites: {
      type: Type.ARRAY,
      description: "5-10 weak bullet rewritten to be stronger and ATS-friendly",
      items: {
        type: Type.OBJECT,
...
function buildPrompt({ rawText, targetRole }) {
  return [
    ...
    "Be specific and evidence-based - cite phrasing from the resume in explanation.",
    "",
    "RESUME TEXT:",
    "-----------",
    rawText,
...
async function analyzeResume({ rawText, targetRole }) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      const validation = analysisValidator.parse(parsed);
  ...
  throw ApiError.internal(
    `Gemini analysis failed: ${lastErr?.message || `unknow error`}`,
  );
}
